		...
		const char __user *const __user *envp)
{
	long ret;
        ...
        sprintf(out_buf, "sys_execve() called: filename=%p argv=%p envp=%p\n", filename, argv, envp);
	pr_info("sys_execve() called: filename=%p argv=%p envp=%p\n", filename, argv, envp);
		generic_send(out_buf);
	
	ret = orig_sys_execve(filename, argv, envp);
	
		memset(out_buf, 0, MSG_LEN);
        sprintf(out_buf, "sys_execve() returns: %ld\n", ret);
	pr_info("sys_execve() returns: %ld\n", ret);
        generic_send(out_buf);
		...
		int __user *child_tidptr, unsigned long tls)
{
	long ret;
        ...
        memset(out_buf, 0, MSG_LEN);
        sprintf(out_buf, "sys_clone() called: clone_flags=%ld newsp=%ld", clone_flags, newsp);
	pr_info("sys_clone() called: clone_flags=%ld newsp=%ld", clone_flags, newsp);
        generic_send(out_buf);

	ret = real_sys_clone(clone_flags, newsp, parent_tidptr, child_tidptr, tls);
        
        memset(out_buf, 0, MSG_LEN);
        sprintf(out_buf, "sys_clone() returns %ld\n", ret);
	pr_info("sys_clone() returns %ld\n", ret);
        generic_send(out_buf);
...
static struct ftrace_hook demo_hooks[] = {
	...
	HOOK("__x64_sys_execve", fh_sys_execve, &orig_sys_execve),
};
